...
def get_user():
    ...
    latest_trades = session.execute("SELECT company, numstock, tradetime FROM trade_history WHERE user=%s AND company IN (%s) ORDER BY tradetime LIMIT 5", parameters=[user, ','.join(COMPANIES)])
